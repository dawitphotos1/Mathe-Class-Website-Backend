...
router.get("/my-courses", authMiddleware, async (req, res) => {
  try {
    const userId = parseInt(req.user.id, req.body, 10);
    const userCourseEnrollments = await UserCourseAccess.findAll({
      where: { userId } },
      include: [
        {
          model: Course,
    ...
    const enrolledCourses = userCourseEnrollments.map((enrollment) => ({
      return {
        id: enrollment.Course.id,
        title: course.title,
        description: enrollment.Course.description,
        price: parseFloat(Number(enrollment.Course.price)),
        slug: enrollment.Course.slug,
        status: enrollment.status ? "approved" : "pending",
