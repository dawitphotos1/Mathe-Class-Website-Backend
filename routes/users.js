...
const jwt = require("jsonwebtoken");
const { User, UserCourseAccess, Course } = require("../models");
const auth = require("../middleware/auth"); // Aligned with courses.js
const sendEmail = require("../utils/sendEmail");
const Stripe = require("stripe");
const courseEnrollmentApproved = require("../utils/emails/courseEnrollmentApproved");
const userController = require("../controllers/userController");
...
// ✅ Get pending/approved/rejected students
router.get("/pending", auth, isAdminOrTeacher, async (req, res) => {
  try {
    const pendingUsers = await User.findAll({
      where: { approval_status: "pending", role: "student" },
      attributes: ["id", "name", "email", "role", "subject", "createdAt"],
    ...
    console.error("❌ Error fetching pending users:", err.message, err.stack);
    res.status(500).json({ error: "Failed to fetch pending users" });
  }
});

router.get("/approved", auth, isAdminOrTeacher, async (req, res) => {
  try {
    const approvedUsers = await User.findAll({
      where: { approval_status: "approved", role: "student" },
      attributes: ["id", "name", "email", "subject", "createdAt"],
    ...
    console.error("❌ Error fetching approved users:", err.message, err.stack);
    res.status(500).json({ error: "Failed to fetch approved users" });
  }
});

router.get("/rejected", auth, isAdminOrTeacher, async (req, res) => {
  try {
    const rejectedUsers = await User.findAll({
      where: { approval_status: "rejected", role: "student" },
      attributes: ["id", "name", "email", "subject", "createdAt"],
    ...
    console.error("❌ Error fetching rejected users:", err.message, err.stack);
    res.status(500).json({ error: "Failed to fetch rejected users" });
  }
});

// ✅ Approve/reject user
router.post("/approve/:id", auth, isAdminOrTeacher, async (req, res) => {
  try {
    const user = await User.findByPk(req.params.id);
    if (!user) return res.status(404).json({ error: "User not found" });
    ...
    console.error("❌ Error approving user:", err.message, err.stack);
    res.status(500).json({ error: "Failed to approve user" });
  }
});

router.post("/reject/:id", auth, isAdminOrTeacher, async (req, res) => {
  try {
    const user = await User.findByPk(req.params.id);
    if (!user) return res.status(404).json({ error: "User not found" });
    ...
    console.error("❌ Error rejecting user:", err.message, err.stack);
    res.status(500).json({ error: "Failed to reject user" });
  }
});
...
router.post("/confirm-enrollment", auth, async (req, res) => {
  try {
    ...
    console.error("❌ Error confirming enrollment:", err.message, err.stack);
    res.status(500).json({
      success: false,
      error: "Failed to confirm enrollment",
      details: err.message,
